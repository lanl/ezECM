...
The purpose of this vignette is to illustrate how one would use the `pval_gen()` function to simulate p-values and then feed these p-values to the `p_agg()` function to illustrate the performance of event categorization techniques.  Alternatively, one could use this vignette to learn how to feed sets of real training p-values along with newly generated p-values to `p_agg()`.
...
The `p_agg()` function cannot use the `event` column of `new.data`.  This column is saved in order to check the accuracy of the method, and then deleted from the `new.data` data frame.
...
We are now ready to use the `p_agg()` function.

# ECM

First, we will use `p_agg()` to fit a model to the training data, and then use this model fit to look at the evidence that the new data is from the distribution of p-values obtained from explosion data.  It is possible to feed both the training data and the new data to the `p_agg()` function simultaneously.  However, the usage illustrated below is more realistic for a real world scenario.
...
```{r pagg_newdata}
new.data.category <- p_agg(x = fit, newdata = new.data)
```

We can then investigate the aggregate p-values for `new.data` belonging to the `"explosion"` distribution.  Organizing and summarizing the output from `p_agg()` allows for a comparison between the aggregate p-value and the true category.  Ideally, the `explosion` column should have an aggregate p-value below the pre-determined threshold when the `new.data.true` column contains `"earthquake"`.

```{r pagg_summary}
categories <- cbind(new.data.category$p_agg, new.data.true)
...
Some ideas I had about functionality to add to the package while typing up this document

*  Export all relevant data or declare S3 object so that additional p-values can be simulated using `pval_gen()` after the first call

*  plot p-values simulated from `pval_gen()` with RDA output.  Allow for transformation, ellipsoidal intervals from RDA, and different colors for training data along with new data.  Code up s.t. plotting user provided p-values can easily be used.

*  Function for importing `*.csv` of user provided p-values, in a way that makes plotting, RDA, and checking newly acquired data easy.

*  Saving output from RDA, in the event that computation time is large, and loading this saved file to use with newly acquired data.
